...
paralelismo oferecido no /hardware/ desses sistemas. Nisso encontramos um grave
problema: com diversas implementações e propostas observamos também clara
diferença de performance entre essas, até entre implementações que seguem a
mesma especificação de API, como é o caso de OpenMP.

A diferença é tal que não se trata mais da qualidade da implementação ditar a
...
gerados, utilizamos a biblioteca LAPACK \cite{lapack1999guide} de versão =3.8.0=
do pacote Netlib[fn:2].
