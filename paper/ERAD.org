...
primeira utilizará ambos os /runtimes/ nativos da ABI do GCC e do LLVM como
também a /runtime/ da biblioteca LIBKOMP baseado em X-Kaapi. A implementação em
StarPU, por fim, será executada nativamente e também através de uma transpilação
da implementação em diretivas OpenMP para StarPU pela biblioteca Kstar. Sendo
assim, nossos objetivos nesse trabalho são comparar o *escalonamento* de
tarefas, observar a *ociosidade* dos trabalhadores de cada /runtime/ e
identificar *anomalias* em quaisquer das bibliotecas testadas.
