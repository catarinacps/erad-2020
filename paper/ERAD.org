...
#+language: pt-br
#+options: H:3 creator:nil timestamp:nil skip:nil toc:nil num:t ^:nil ~:~ date:nil title:nil
...
bibliotecas e /runtimes/ de programação paralelas. Essas implementações suportam
...
Avaliaremos o desempenho e comportamento de uma aplicação que
implementa uma fatoração QR de uma matriz densa de implementações
utilizando diretamente as diretivas de tarefas do OpenMP com os
/runtimes/ nativos de ambos GCC e LLVM e também utilizando o /runtime/ da
biblioteca LIBKOMP baseado em X-Kaapi, de uma implementação pura em
StarPU, de uma transpilação da implementação em diretivas OpenMP para
StarPU pela biblioteca Kstar. Logo, nossos objetivos nesse trabalho
são comparar o *escalonamento* de tarefas, observar a *ociosidade* dos
trabalhadores de cada /runtime/ e identificar *anomalias* em quaisquer das
bibliotecas testadas.

# ...
# <insira paragrafo sobre related work aqui>
# ...
...
[[tab:plataformas][Tabela 1]]. Listamos em cada uma dessas o seu identificador, o(s)
processador(es), a quantidade e níveis de memória /cache/ e quantidade de
memória /RAM/.
...
|---------+-----+---------------------------------------+--------------------+------------|
| =draco= | $5$ | $2 \times 8$ Xeon E5 2640 v2 2.5GHz   | 32KB/256KB/20MB    | 64GB DDR3  |
| =cei=   | $7$ | $2 \times 12$ Xeon Silver 4116 2.1GHz | 32KB/1024KB/16.5MB | 93GB DDR4  |
| =hype=  | $4$ | $2 \times 10$ Xeon E5 2650 v3 2.3GHz  | 32KB/256KB/25MB    | 128GB DDR4 |
|---------+-----+---------------------------------------+--------------------+------------|
...
#+latex: \noindent
*Versão das Bibliotecas e Binários*: A ferramenta utilizada para a
compilação de todos os binários foi o /frontend/ para a linguagem C do
sistema de compiladores GCC, versão =8.3.0= \cite{gnu2018manual}. As
bibliotecas padrão utilizadas foram, portanto, as distribuídas com
esse /release/ do compilador. O /runtime/ de OpenMP utilizado do projeto
LLVM foi a versão distribuída com o lançamento =6.0.0= do /frontend/ Clang
...
utilizada foi a versão estável =1.3.1=. Para todos os binários gerados,
utilizamos a biblioteca LAPACK \cite{lapack1999guide} de versão =3.8.0=,
distribuída no pacote Netlib[fn:2]. Para as ferramentas LIBKOMP
\cite{broquedis2012libkomp} e KStar \cite{7912335}, foram utilizadas
as versões em desenvolvimento, /branch/ ~libkomp~, /commit hash/
32781b6dab10b1b51a07ef074078f28efd8f9b4e e /branch/ ~master~, /commit hash/
bf6af54e57bad13004d21fa2ab8297922d6af151.

# falta info sobre libkomp e kstar, que não possuem versões estáveis. como citar?
