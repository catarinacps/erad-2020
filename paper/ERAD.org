...
Avaliaremos o desempenho e comportamento de uma aplicação que implementa uma
fatoração QR de uma matriz densa de implementações utilizando diretamente as
diretivas de tarefas do OpenMP com os /runtimes/ nativos de ambos GCC e LLVM e
também utilizando o /runtime/ da biblioteca LIBKOMP baseado em StarPU, de uma
implementação em StarPU, de uma transpilação da implementação em diretivas
OpenMP para StarPU pela biblioteca Kstar. Logo, nossos objetivos nesse trabalho
