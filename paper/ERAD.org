...
dependências dos /runtimes/, desenvolvemos um /script/ que abstrai detalhes a
plataforma e permite a fácil reprodução de um experimento.
...
utilizando chamadas conforme a especificação OMPT =4.5=. O rastreamento das
biblioteca e compilador StarPU e KStar_{StarPU} foi realizado através da
biblioteca FxT =0.3.5= e o rastreamento das versões utilizando LIBKOMP foi
realizado pelo próprio /runtime/, que implementa chamadas à /API/ OMPT.
...
ambos LIBKOMP_clang quanto KStar_{StarPU} apresentam ociosidade média
