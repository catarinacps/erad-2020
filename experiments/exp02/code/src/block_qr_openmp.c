...
int main(int argc, char const* argv[])
{
    ...
    unsigned long int ib, ib2, jb, kb, zb, zit; // iterators for loop control
...
#pragma omp single
        {
            ...
            for (k = 0; k < min(b, mb); k++) {
                ...
                kb = k * nb * n + k * nb;
// #pragma omp task depend(inout: A[kb:kb], A[kb+1:kb+1]) depend(out: T[0:tsize]) priority(4)
#ifdef __clang__
#pragma omp task depend(inout                                      \
                        : A [kb:kb], A [kb + 1:kb + 1]) depend(out \
                                                               : T [0:tsize]) taskname("dgeqrt")
#elif __GNUC__
#pragma omp task depend(inout                                      \
                        : A [kb:kb], A [kb + 1:kb + 1]) depend(out \
                                                               : T [0:tsize])
                ...
                for (j = k + 1; j < b; j++) {
                    jb = k * nb * n + j * nb;
// #pragma omp task depend(in: A[kb:kb], T[0:tsize]) depend(inout: A[jb:jb+nb]) priority(2)
#ifdef __clang__
#pragma omp task depend(in                                     \
                        : A [kb:kb], T [0:tsize]) depend(inout \
                                                         : A [jb:jb + nb]) taskname("dlarfb")
#elif __GNUC__
#pragma omp task depend(in                                     \
                        : A [kb:kb], T [0:tsize]) depend(inout \
                                                         : A [jb:jb + nb])
                ...
                for (i = k + 1; i < mb; i++) {
                    ib = i * nb * n + k * nb;
// #pragma omp task depend(inout: A[kb+1:kb+1], A[ib:ib+nb]) depend(out: T2[0:tsize]) priority(3)
#ifdef __clang__
#pragma omp task depend(inout                                           \
                        : A [kb + 1:kb + 1], A [ib:ib + nb]) depend(out \
                                                                    : T2 [0:tsize]) taskname("dtpqrt")
#elif __GNUC__
#pragma omp task depend(inout                                           \
                        : A [kb + 1:kb + 1], A [ib:ib + nb]) depend(out \
                                                                    : T2 [0:tsize])
...
// #pragma omp task depend(inout: A[jb:jb+nb], A[ib2:ib2+nb]) depend (in: A[ib:ib+nb], T2[0:tsize]) priority(1)
#ifdef __clang__
#pragma omp task depend(inout                                         \
                        : A [jb:jb + nb], A [ib2:ib2 + nb]) depend(in \
                                                                   : A [ib:ib + nb], T2 [0:tsize]) taskname("dtpmqrt")
#elif __GNUC__
#pragma omp task depend(inout                                         \
                        : A [jb:jb + nb], A [ib2:ib2 + nb]) depend(in \
