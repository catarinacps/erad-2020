...
- [[#experiments][Experiments]]
  - [[#preamble][Preamble]]
    ...
    - [[#doing-data-analysis][Doing data analysis]]
...
In order to use the R tools I've used, please run the following source block to
load them up:
...
I'll install dependencies in =$HOME/Installs/spack= directory but you can put
them anywhere really, just pass to the =launch.sh= script the path to the
directory you'll use.

So, let's first start by cleaning up the directory I'll use to store the
symbolic links:

#+begin_src bash :exports both :results output
pushd $HOME

rm -rf Installs/spack
mkdir Installs/spack

popd
#+end_src

...and lets also install =spack= at =$HOME/spack=
...
Something about this experiment!
