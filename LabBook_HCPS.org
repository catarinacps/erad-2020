...
gantt_scorep <- function (dtx) {
    ...
    # End of last task
    makespan = dta %>% filter(Kernel == "dgeqrt") %>% .$End %>% max
    makespan_y = (dta %>% .$WorkerID %>% unique %>% length) / 2

    dti <- dta %>% group_by(WorkerID) %>% summarize(Idleness = (1-(sum(End-Start)/makespan))*100)
...
gantt_ompt <- function (dtx) {
    ...
    # time zero (start of  first task in the DAG)
    zero <- dtx %>% filter(Operation == "lapack_dgeqrt") %>% .$time_start %>% min
    dtx %>% mutate(time_start=time_start-zero, time_end=time_end-zero) -> dty

    dty %>% mutate(Thread=as.factor(Thread), Operation=as.factor(Operation)) %>% mutate(Operation=factor(Operation, levels=mixedsort(levels(Operation)))) -> dta

    # time_end of last task
    makespan = dta %>% filter(Operation == "lapack_dgeqrt") %>% .$time_end %>% max
    makespan_y = (dta %>% .$Thread %>% unique %>% length) / 2

    dti <- dta %>% group_by(Thread) %>% summarize(Idleness = (1-(sum(time_end-time_start)/makespan))*100)
    
    ggplot(data=dta, aes(x=time_start, y=Thread)) +
