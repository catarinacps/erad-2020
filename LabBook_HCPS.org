...
while read -r runtime matrix num_blocks; do
    ...
    if [[ $runtime = starpu ]] || [[ $runtime = kstar_starpu ]]; then
        export STARPU_FXT_PREFIX=$(readlink -f results/$run_id)/
        LD_LIBRARY_PATH+=:$HDF5_PATH/lib
        LD_LIBRARY_PATH+=:$STARPU_PATH/lib
    elif [[ $runtime = kstar_starpu ]]; then
        LD_LIBRARY_PATH+=:$KSTAR_PATH/lib
        ...
        export SCOREP_EXPERIMENT_DIRECTORY=$(readlink -f results/$run_id)
    fi
...
**** Reading Feather
 #+header: :var rawDirSimulated="./experiments/exp02/results-draco1-wip"
 #+begin_src R :results output :exports both :session Rvz :eval no-export
blockQrStarpuKstar <- 
    grep("starpu", list.dirs(path=rawDirSimulated, full.names=TRUE, recursive=FALSE), value=TRUE) %>%   
    lapply(function(fl){ 
...
names(blockQrStarpuKstar) <- 
    grep("starpu", list.dirs(path=rawDirSimulated, full.names=TRUE, recursive=FALSE), value=TRUE) %>%
    lapply(function(fl){
        nm <- basename(fl) %>% strsplit("-") %>% unlist()
        nm[3]
    })
 #+end_src
...
**** Plots
 #+begin_src R :results output graphics :file qr-starpu-kstar-starvz.png :exports both :width 1600 :height 1500 :session Rvz :eval no-export
pajer$st$abe$active=TRUE

pajer$lackready$active = TRUE

pajer$pmtool$state$active = TRUE
pajer$pmtool$state$sched = "dmdas"

pajer$pmtool$kiteration$active = FALSE

pajer$pmtool$bounds$active = TRUE
pajer$pmtool$bounds$alg = blockQrStarpuKstar[[1]]$Pmtool %>% filter(!is.na(Time)) %>% .$Alg %>% unique
