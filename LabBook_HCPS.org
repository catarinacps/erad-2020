...
****** Full View Chart
******* Execution "kstar-starpu-8192-128"
 #+header: :var dep1=customConfig
 #+begin_src R :results output graphics :file img/trace-qr-kstar-starpu-8192-128-starvz.png :exports both :width 1400 :height 1200 :session R :eval no-export
   the_master_function(blockQrStarpuKstar[["kstar-starpu-8192-128"]])
 #+end_src


******* Execution "starpu-8192-128"
 #+header: :var dep1=customConfig
 #+begin_src R :results output graphics :file img/trace-qr-starpu-8192-128-starvz.png :exports both :width 1400 :height 1200 :session R :eval no-export
   the_master_function(blockQrStarpuKstar[["starpu-8192-128"]])
 #+end_src
...
****** Full View Chart
******* Execution "libkomp-clang-8192-128"
 #+begin_src R :results output graphics :file img/trace-qr-libkomp-clang-8192-128-libkomp.png :exports both :width 1400 :height 800 :session R :eval no-export
   gantt_libkomp(blockQrLibkomp[["libkomp-clang-8192-128"]])
 #+end_src


******* Execution "libkomp-clang-8192-256"
 #+begin_src R :results output graphics :file img/trace-qr-libkomp-clang-8192-256-libkomp.png :exports both :width 1400 :height 800 :session R :eval no-export
   gantt_libkomp(blockQrLibkomp[["libkomp-clang-8192-256"]])
 #+end_src
...
gantt_scorep <- function (dtx) {
    ...
    dti <- idleness_scorep(dta , makespan)
...
for file in `find $rawPath -maxdepth 1 -type d -name "*scorep*"`;  do
...
#+begin_src R :results output :session R :exports both
blockQrScorep <-
    grep("scorep", list.dirs(path=rawDirExp, full.names=TRUE, recursive=FALSE), value=TRUE) %>%
    lapply(function(fl){
        print(paste0("Working on ", fl))
...
****** Full View Chart
******* Execution "scorep-8192-256"
 #+begin_src R :results output graphics :file img/trace-qr-scorep-8192-256-scorep.png :exports both :width 1400 :height 800 :session R :eval no-export
   gantt_scorep(blockQrScorep[["scorep-8192-256"]])
 #+end_src
...
#+begin_src R :results output :session R :exports both
blockQrOmpt <-
    grep("ompt", list.dirs(path=rawDirExp, full.names=TRUE, recursive=FALSE), value=TRUE) %>%
    lapply(function(fl){
        print(paste0("Working on ", fl))
...
****** Full View Chart
******* Execution "ompt-8192-256"
 #+begin_src R :results output graphics :file img/trace-qr-ompt-8192-256-ompt.png :exports both :width 1400 :height 800 :session R :eval no-export 
   gantt_ompt(blockQrOmpt[["ompt-8192-256"]])
 #+end_src
...
***** Code
      ...
      plot_duration <- function(dta){
          dta %>% filter(Task %in% c("dgeqrt", "dtpqrt", "dlarfb", "dtpmqrt")) %>% group_by(Task, Runtime) %>% summarize(Mean=mean(Duration), Median=median(Duration), Sd=sd(Duration)) %>%
...
***** Merge all
      ...
      bind_rows(
          lapply(blockQrStarpuKstar %>% names, function(x){
              ...
              blockQrStarpuKstar[[x]]$State %>% select(Value, ResourceId, Duration) %>% rename(Task=Value, Resource=ResourceId) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) %>% mutate(Task=str_replace(Task, "lapack_", ""))
          }),
          lapply(blockQrLibkomp %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrLibkomp[[x]] %>% select(Name, Resource, Duration) %>% rename(Task=Name) %>% mutate(Resource=as.character(Resource)) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)])
          }),
          lapply(blockQrScorep %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrScorep[[x]] %>% select(Kernel, WorkerID, Duration) %>% rename(Task=Kernel) %>% mutate(Resource=as.character(WorkerID)) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)])
          }),
          lapply(blockQrOmpt %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrOmpt[[x]] %>% select(Operation, Thread, time_elapsed) %>% rename(Task=Operation, Duration=time_elapsed) %>% mutate(Resource=as.character(Thread)) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)])  %>% mutate(Task=str_replace(Task, "lapack_", ""))
          })
      ) -> dt_duration
      #+end_src
...
***** Merge all
      ...
      bind_rows(
          lapply(blockQrStarpuKstar %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrStarpuKstar[[x]]$State %>% calculate_resource_idleness(max_only = FALSE) %>% select(-ResourceType, -Node, -ResourceId, -Height, -End) %>% rename(Resource=Position)  %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) 
          }),
          lapply(blockQrLibkomp %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrLibkomp[[x]] %>% idleness_libkomp() %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) 
          }),
          lapply(blockQrScorep %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrScorep[[x]] %>% idleness_scorep()  %>% rename(Resource=WorkerID) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) 
          }),
          lapply(blockQrOmpt %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrOmpt[[x]] %>% idleness_ompt() %>% rename(Resource=Thread)  %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) 
          })
...
***** Plot - Idleness All
      #+begin_src R :results output graphics :file img/idleness-all.png :exports both :width 600 :height 400 :session R
       dt_idleness %>% plot_all_idleness
      #+end_src


***** Plot - Idleness Average
      #+begin_src R :results output graphics :file img/idleness-average.png :exports both :width 600 :height 400 :session R
       dt_idleness %>% plot_idleness_average
      #+end_src
...
***** Merge all
      ...
      bind_rows(
          lapply(blockQrStarpuKstar %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrStarpuKstar[[x]]$State %>% select(Value, Start) %>% rename(Task=Value) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) %>% mutate(Task=str_replace(Task, "lapack_", "")) %>% filter(grepl("dgeqrt", Task))
          }),
          lapply(blockQrLibkomp %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrLibkomp[[x]] %>% select(Name, Start) %>% rename(Task=Name) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) %>% filter(grepl("dgeqrt", Task))
          }),
          lapply(blockQrScorep %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrScorep[[x]] %>% select(Kernel, Start) %>% rename(Task=Kernel) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)]) %>% filter(grepl("dgeqrt", Task))
          }),
          lapply(blockQrOmpt %>% names, function(x){
              nm <- x %>% strsplit("-") %>% unlist()
              blockQrOmpt[[x]] %>% select(Operation, time_start) %>% rename(Task=Operation, Start=time_start) %>% mutate(Runtime=(nm[0:(length(nm)-2)] %>% paste(collapse ='-')), Matrix=nm[length(nm)-1], Block=nm[length(nm)])  %>% mutate(Task=str_replace(Task, "lapack_", "")) %>% filter(grepl("dgeqrt", Task))
          })
      ) -> dt_start
      #+end_src
