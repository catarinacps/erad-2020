...
** DONE 1 - Overview - makespan only                                 :EXP01:
CLOSED: [2020-01-25 sáb 01:47]
- State "DONE"       from "WAITING"    [2020-01-25 sáb 01:47]
...
fig_1 %>%
  ...
  geom_errorbar(aes(ymin = avg_time - error,
                ...
                width = 0.5,
                position=position_dodge(width = 1)) +
  labs(y = "Tempo médio de execução [seg]",
       color = "Runtime",
       shape = "Runtime") +
...
plot_duration <- function(dta){
  dta %>% filter(Task %in% c("dgeqrt", "dtpqrt", "dlarfb", "dtpmqrt")) %>%
    ...
    labs(y = "Duração média da tarefa [sec]",
         x = "Runtime") +
    theme_bw() +
    theme(legend.position = "bottom")
}
...
bind_rows(
    lapply(blockQrStarpuKstar %>% names, function(x){
      ...
      blockQrStarpuKstar[[x]]$State %>%
        select(Value, ResourceId, Duration) %>%
        rename(Task=Value, Resource=ResourceId) %>%
        mutate(Runtime=(nm[2:(length(nm)-2)] %>% paste(collapse ='-')),
               Matrix=nm[length(nm)-1],
               Block=nm[length(nm)],
               Platform=nm[1]) %>%
        mutate(Task=str_replace(Task, "lapack_", ""))
    }),
    ...
    lapply(blockQrScorep %>% names, function(x){
      ...
      blockQrScorep[[x]] %>%
        select(Kernel, WorkerID, Duration) %>%
        rename(Task=Kernel) %>%
        mutate(Resource=as.character(WorkerID)) %>%
        mutate(Runtime=(nm[2:(length(nm)-2)] %>% paste(collapse ='-')),
               Matrix=nm[length(nm)-1],
               Block=nm[length(nm)],
               Platform=nm[1])
    ...
    lapply(blockQrOmpt %>% names, function(x){
      ...
      blockQrOmpt[[x]] %>%
        select(Operation, Thread, time_elapsed) %>%
        rename(Task=Operation, Duration=time_elapsed) %>%
        mutate(Resource=as.character(Thread)) %>%
        mutate(Runtime=(nm[2:(length(nm)-2)] %>% paste(collapse ='-')),
               Matrix=nm[length(nm)-1],
               Block=nm[length(nm)],
               Platform=nm[1]) %>%
...
#+begin_src R :results output :session R :exports both :tangle (concat (hcps/get-user-server) "/scratch/hcpsilva/script.R")
plot_all_idleness <- function(dta) {
  dta %>%
    ...
    scale_y_continuous(expand = expand_scale(mult = c(0, 0.05)),
                       breaks = scales::pretty_breaks(n = 8)) +
    theme_bw() +
    theme(text = element_text(family = "Palatino", size = 18),
          strip.text = element_text(size = 16),
          legend.position = "right",
          plot.margin = unit(c(0, 10, 0, 0), "mm"))
  ggsave("temp.png", width = 30, height = 8, units = "cm")
}
...
idleness_libkomp <- . %>%
  ...
  group_by(Resource) %>%
  summarize(Idleness = (1 - (sum(End - Start)/makespan_libkomp(.))) * 100)

makespan_scorep <- . %>%
  filter(Kernel == "dgeqrt") %>%
  .$End %>%
  max

idleness_scorep <- . %>%
  group_by(WorkerID) %>%
  summarize(Idleness = (1 - (sum(End - Start) / makespan_scorep(.))) * 100)

makespan_ompt <- . %>%
  filter(Operation == "lapack_dgeqrt") %>%
  .$time_end %>%
  max

idleness_ompt <- . %>%
  group_by(Thread) %>%
  summarize(Idleness = (1 - (sum(time_end - time_start) / makespan_ompt(.))) * 100)
...
bind_rows(
  lapply(blockQrStarpuKstar %>% names, function(x){
    ...
    blockQrStarpuKstar[[x]]$State %>%
      calculate_resource_idleness(max_only = FALSE) %>%
      select(-ResourceType, -Node, -ResourceId, -Height, -End) %>%
      rename(Resource=Position) %>%
      mutate(Runtime=(nm[2:(length(nm)-2)] %>% paste(collapse ='-')),
             Matrix=nm[length(nm)-1],
             Block=nm[length(nm)],
             Platform=nm[1])
...
dt_idleness %>% filter(Platform=="cei", Runtime!="libkomp-gcc", Block == 64) %>% plot_all_idleness
#+end_src

#+begin_src bash :exports both :results output
scp cei2:/scratch/hcpsilva/temp.png img/idleness-all-cei.png
#+end_src
...
bind_rows(
    ...
    lapply(blockQrOmpt %>% names, function(x){
      ...
      blockQrOmpt[[x]] %>%
        select(Operation, time_start) %>%
        rename(Task=Operation, Start=time_start) %>%
        mutate(Runtime=(nm[2:(length(nm)-2)] %>% paste(collapse ='-')),
               Matrix=nm[length(nm)-1],
               Block=nm[length(nm)],
               Platform=nm[1]) %>%
...
#+begin_src R :results output :exports both :tangle (concat (hcps/get-user-server) "/scratch/hcpsilva/script.R")
plot_dgeqrt_start <- function(dta) {
  dta %>%
    group_by(Runtime, Matrix, Block) %>%
    arrange(Start) %>%
    mutate(i = seq(n())) %>%
    ggplot(aes(x = Start, y = i, color = Runtime, shape = Runtime)) +
    geom_point() +
    facet_grid(Block ~., scales = "free") +
    labs(y = "DGEQRT",
         x = "Início da tarefa [sec]") +
    scale_y_reverse(expand = expand_scale(mult = c(0, 0.05)),
                    breaks = scales::pretty_breaks(n = 8)) +
    theme_bw() +
    theme(text = element_text(family = "Palatino", size = 18),
          strip.text = element_text(size = 16),
...
plot_dgeqrt_start(dt_start %>% filter(Platform == "cei", Runtime != "libkomp-gcc", Block == 64))
#+end_src
...
#+begin_src bash :exports both :results output
scp cei2:/scratch/hcpsilva/temp.png img/dgeqrt-start-cei.png
#+end_src
