...
***** Merge all
      #+begin_src R :results output :session R :exports both
      bind_rows(
          blockQrStarpuKstar[[1]]$State %>% select(Value, ResourceId, Duration) %>% rename(Task=Value, Resource=ResourceId) %>% mutate(Runtime="KStar-StarPU"),
          blockQrStarpuKstar[[2]]$State %>% select(Value, ResourceId, Duration) %>% rename(Task=Value, Resource=ResourceId) %>% mutate(Runtime="StarPU") %>% mutate(Task=str_replace(Task, "lapack_", "")),
          #blockQrLibkomp[[1]] %>% select(Name, Resource, Duration) %>% rename(Task=Name) %>% mutate(Resource=as.character(Resource), Runtime="Libkomp-Clang")
      ) -> dt_duration
      #+end_src
