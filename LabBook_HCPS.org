...
data_dir %>%
  list.files(pattern = glob2rx("*cei*.tar.gz"), full.names = TRUE) %>%
  lapply(., Curry(untar, exdir = str_replace(., ".tar.gz", "/")) -> ret
#+end_src

#+begin_src bash :exports both :results output :var EXP_DATA="experiments/exp01/data"
echo $EXP_DATA
mkdir $EXP_DATA/cei
for file in $(find $EXP_DATA -type f -name "*cei*.tar.gz"); do
    echo $file
    tar xzf $file -C $EXP_DATA/cei
done
#+end_src

#+RESULTS:
: experiments/exp01/data
: experiments/exp01/data/exp01_cei7_48_data.tar.gz
: experiments/exp01/data/exp01_cei4_48_data.tar.gz
: experiments/exp01/data/exp01_cei8_48_data.tar.gz
: experiments/exp01/data/exp01_cei6_48_data.tar.gz
: experiments/exp01/data/exp01_cei5_48_data.tar.gz
: experiments/exp01/data/exp01_cei2_48_data.tar.gz
: experiments/exp01/data/exp01_cei3_48_data.tar.gz
...
fig_1 %>%
  ...
  geom_point(size = 1, position = position_dodge2()) +
  geom_errorbar(aes(ymin = avg_time - error,
                ymax = avg_time + error),
                position = position_dodge2()) +
  facet_grid(block_size ~ ., scales = "free") +
  labs(x = "Number of tasks",
       y = "Average task time (seconds)",
       fill = "Utilized runtime") +
  ylim(0, NA) +
  theme_bw() +
  theme(text = element_text(family = "Palatino", size = 16)) -> plot1
#+end_src
