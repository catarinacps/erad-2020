...
gantt_scorep <- function (dtx) {
    ...
    ggplot(data=dta, aes(x=date(Start), y=WorkerID)) +
        ...
        annotate("text", x = date(makespan), y = makespan_y, label = round(makespan, 2), angle=90) +
        geom_label(data=dti, aes(x=date(makespan*(-0.05)), y=WorkerID, label=paste(round(Idleness, 2), "%"))) +
        xlab("Time") +
        ylab("Application Threads") +
        theme_bw() +
        theme (legend.position = "bottom") +
...
#+begin_src R :results output :session R :exports both
read_csv("./experiments/exp02/results-draco1-wip/scorep_8192_256/paje.libgomp.csv", col_names=FALSE, col_types=cols()) %>%
    select(-X1, -X3) %>%
    rename(Worker = X2,
           Start = X4,
           End = X5,
...
df.post.3 -> df.libgomp
#+end_src


***** Plot
      #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 1350 :height 400 :session R
gantt_scorep(df.libgomp)
      #+end_src
